...
SmartApp::SmartApp(BIF::IByteStream& stream, Rect screen, AppTiming timing,
...
void SmartApp::startDiscover() noexcept {
    // Сканирование panel `.id` по страницам; результат — `_table`, затем `onDiscoverComplete`.
...
void SmartApp::applyFromTable() noexcept {
    for (uint16_t i = 0u; i < _table.count; ++i) {
        ...
        MISC::ObjRegistry<Component, uint8_t>& reg = pg->storage();
        ...
        if (st != MISC::RegStatus::Ok && st != MISC::RegStatus::SwappedWithOccupiedSlot)
            onStatus(appErrorFrom(st), Route{pg->ID, c->id()});
    }
}
...
void SmartApp::discoverEnqueueGetId(uint8_t compiled_id) noexcept {
    IPage* const pg = getPage(_page_index);
    if (pg == nullptr)
        return;
    ...
    enqueue(Transaction{cmd::Get::numeric(target), Route::compIdMapPoll(),
...
void SmartApp::discoverOnPageChanged(uint8_t page) noexcept {
...
uint8_t SmartApp::discoverComponentCount(uint8_t page_index) noexcept {
    IPage* const pg = getPage(page_index);
    if (pg == nullptr)
        return 0u;
...
void SmartApp::dispatchEvent(const Message& m) noexcept {
    // `sendme` / смена страницы — `0x66`; без этого SwitchPage не видит `currentPage`.
...
void SmartApp::onPageChange(const msg::evPage& e) noexcept {
    IAppUI::onPageChange(e);
    discoverOnPageChanged(e.page);
}

void SmartApp::onTimeout(const Transaction& active) noexcept {
    if (active.route.isCompIdMapPoll()) {
        if (_phase == DiscoverPhase::GetId)
