...
const Record* Table::find(uint8_t page_id, uint8_t compiled_id) const noexcept {
    for (uint16_t i = 0u; i < count; ++i) {
        const Record& r = records[i];
        if (r.page_id == page_id && r.compiled_id == compiled_id)
            return &r;
    }
    return nullptr;
}

Record* Table::find(uint8_t page_id, uint8_t compiled_id) noexcept {
    return const_cast<Record*>(static_cast<const Table*>(this)->find(page_id, compiled_id));
}

bool Table::upsert(uint8_t page_id, uint8_t compiled_id, uint8_t panel_id) noexcept {
    if (!isBound() || compiled_id < kFirstCompId || panel_id == kPageCompId)
        return false;

    if (Record* const existing = find(page_id, compiled_id)) {
