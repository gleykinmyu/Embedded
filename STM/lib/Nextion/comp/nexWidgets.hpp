...
#include "../core/nexColor.hpp"
#include "nexComponents.hpp"

namespace nex {

/** Режим `sta` у Variable: число или строка (NIS / Editor). */
...
/** sta(Number|String); val|txt; txt_maxl (строковый режим) */
class Variable : public Component {
public:
    VariableMode sta{}; /**< Режим int/str (атрибут `sta` у va). */
    int32_t val{};      /**< Значение в числовом режиме (`val`). */
    uint16_t txt_maxl{};
    std::array<char, 256> txt{}; /**< Буфер под строковый режим (`txt`). */
