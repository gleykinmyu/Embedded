...
bool CBoard::initDmxPins() noexcept
{
    CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
    DWT->CYCCNT = 0;
    DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk;

    dmx.InitPins(dmxTx, GPIO::PortB::pin<11>);
    if (!dmx.setFrameFormat(UART::Frame::_8N2()))
        return false;
    dmxDe.Init(GPIO::Mode::Output, GPIO::Pull::None, GPIO::Speed::VeryHigh);
    dmxDe.Write(false);
    return true;
}
...
void delayUs(uint32_t us) noexcept
{
    if (us == 0u)
        return;
    const uint32_t kHz = SystemCoreClock / 1'000'000u;
    const uint32_t cycles = us * (kHz == 0u ? 1u : kHz);
