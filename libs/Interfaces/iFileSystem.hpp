 ...
 *
 * Размеры имён привязаны к `ffconf.h` (`_USE_LFN` / `_MAX_LFN`).
 */
...
/** Длина буфера basename (+NUL), как у `FILINFO::fname`. */
#if _USE_LFN
inline constexpr std::size_t kDirNameSize = static_cast<std::size_t>(_MAX_LFN) + 1u;
#else
inline constexpr std::size_t kDirNameSize = 13u; /**< 8.3 + NUL. */
#endif
